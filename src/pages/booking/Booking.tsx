...
const Booking = () => {
  ...
  const handleConfirm = async () => {
    ...
    let clarityHubLink = '';
    try {
      const clientRes = await fetch('/.netlify/functions/create-client-from-booking', {
        method: 'POST',
      ...
      if (clientRes.ok) {
        ...
        // Store for email reference
        clarityHubLink = `https://doxaandco.co/clarityb/login?email=${encodeURIComponent(form.email)}&tempPassword=${encodeURIComponent(data.tempPassword || '')}`;
        console.log('[booking] ClarityHub account created:', data);
      } else {
        console.warn('[booking] Failed to create ClarityHub account:', await clientRes.text());
      }
    } catch (err) {
      console.error('[booking] ClarityHub creation error:', err);
      // Non-blocking — continue with booking even if ClarityHub creation fails
    }

    // ── Add to Mailchimp for email nurture ──────────────────────────────
    try {
      ...
      console.log('[booking] Added to Mailchimp for nurture');
    } catch (err) {
      console.error('[booking] Mailchimp sync error (non-blocking):', err);
    }

    // ── Handle payment (if fee is required) ──────────────────────────────
    if (selectedType.fee && selectedType.fee !== 'free') {
      // Extract amount (e.g., "$350" → 35000 cents)
      const amountMatch = selectedType.fee.match(/\$?([\d.]+)/);
      if (amountMatch) {
        const amount = Math.round(parseFloat(amountMatch[1]) * 100);
        try {
          const checkoutRes = await fetch('/.netlify/functions/create-booking-checkout', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              email: form.email,
              ...
              meetingType: selectedType.title,
              amount,
              orgId: clarityHubLink ? new URL(clarityHubLink).searchParams.get('email') : undefined,
            }),
          });
          ...
          if (checkoutRes.ok) {
            ...
            console.warn('[booking] Failed to create checkout:', await checkoutRes.text());
          }
        } catch (err) {
